"""
Response completion detector for DeepSeek (chat.deepseek.com).
...
async def wait_for_response_complete(
    ...
    """Wait until DeepSeek finishes generating its response."""
    timeout = timeout_ms or Config.RESPONSE_TIMEOUT
    log.info(f"Waiting for DeepSeek response (timeout={timeout}ms)...")

    poll_interval = Config.POLL_INTERVAL_MS / 1000
    elapsed = 0.0
    stop_sels = DeepSeekSelectors.STOP_BUTTON
    ...
    if stop_visible:
        log.debug("Stop button visible — waiting for DeepSeek to finish...")
        while elapsed * 1000 < timeout:
            ...
            for sel in stop_sels:
                ...
                if el and await el.is_visible():
                    ...
                    break
            if not visible:
                log.info("DeepSeek response complete — stop button gone")
    ...
    log.info("DeepSeek: falling back to text-stability detection...")
    last_text = ""
    stable_count = 0
    required_stable = 3
    ...
    while elapsed * 1000 < timeout:
        ...
        if text and text == last_text and not is_incomplete_response_text(text):
            ...
            if stable_count >= required_stable:
                log.info("DeepSeek response text stabilized")
                return True
        else:
            stable_count = 0
            last_text = text
    ...
    log.warning(f"DeepSeek response timeout after {int(elapsed)}s")
    return False
...
async def extract_last_response_via_copy(
    ...
    """Extract the latest assistant message from DeepSeek."""
    # Try copy button
    try:
        ...
        log.debug(f"DeepSeek copy button failed: {e}")
