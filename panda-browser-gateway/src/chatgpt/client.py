...
class ChatGPTClient:
    ...
    # ChatGPT model UI display names (what appears in the dropdown)
    _MODEL_DISPLAY = {
        "gpt-4o": "GPT-4o",
    ...
    async def select_model(self, model_id: str) -> bool:
        """
        Select a model in the ChatGPT web UI by clicking the model picker.

        Returns True if selection succeeded (or was already active),
        False if the model could not be selected (non-fatal — current model used).
        """
        # Resolve alias
        ...
        if not display:
            log.debug(f"select_model: unknown model '{model_id}' — skipping UI selection")
            return False

        if self._current_model == resolved:
            log.debug(f"select_model: '{resolved}' already active")
            return True

        log.info(f"select_model: switching to '{resolved}' (display='{display}')")

        # Selectors for the model picker button (ChatGPT UI varies)
        picker_selectors = [
            "button[data-testid='model-switcher-dropdown-button']",
            "[data-testid='model-switcher']",
            "button[aria-label='Model selector']",
            "button[aria-haspopup='listbox']",
            # Fallback: any button whose text contains the current model name
            f"button:has-text('GPT')",
            f"button:has-text('o1')",
            f"button:has-text('o3')",
        ]
        ...
        for sel in picker_selectors:
            try:
                ...
                if el and await el.is_visible():
                    ...
                    opened = True
                    log.debug(f"select_model: picker opened via '{sel}'")
                    break
            except Exception:
                continue

        if not opened:
            log.warning("select_model: could not open model picker — using current model")
            return False

        # Now find and click the target model option in the dropdown
        option_selectors = [
            f"[data-testid='model-option-{resolved}']",
            f"[data-testid='model-option-{model_id}']",
            f"li:has-text('{display}')",
            f"div[role='option']:has-text('{display}')",
            f"button:has-text('{display}')",
            f"[role='menuitem']:has-text('{display}')",
        ]
        ...
        log.warning(f"select_model: option '{display}' not found in dropdown")
        return False
