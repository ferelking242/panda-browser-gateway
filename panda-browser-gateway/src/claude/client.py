...
class ClaudeClient:
    ...
    _MODEL_ALIASES = {
        "claude-browser": "claude-3-5-sonnet-20241022",
    ...
    async def select_model(self, model_id: str) -> bool:
        """
        Select a model in the Claude web UI by clicking the model picker.

        Returns True on success, False if selection failed (non-fatal).
        """
        resolved = self._MODEL_ALIASES.get(model_id, model_id)
        display = self._MODEL_DISPLAY.get(resolved, "")

        if not display:
            log.debug(f"select_model: unknown model '{model_id}' — skipping")
            return False
        ...
        if not opened:
            log.warning("select_model: could not open Claude model picker")
            return False
        ...
        log.warning(f"select_model: option '{display}' not found in Claude picker")
        return False
