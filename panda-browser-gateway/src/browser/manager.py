...
class BrowserManager:
    ...
    def __init__(self, user_data_dir: Path | None = None) -> None:
        ...
        # Allow pool slots to use separate data dirs (browser_data_0, _1, _2…)
        self._user_data_dir: Path = user_data_dir or Config.BROWSER_DATA_DIR
    ...
    async def start(self) -> Page:
        ...
        _cleanup_stale_locks(self._user_data_dir)
        ...
        # Allow overriding the Chromium binary via env var (needed on Replit/Nix
        # where the patchright-downloaded binary has missing shared libs).
        import os as _os
        _custom_exe = _os.environ.get("PLAYWRIGHT_CHROMIUM_EXECUTABLE_PATH") or \
                      _os.environ.get("CHROMIUM_EXECUTABLE_PATH")
        ...
        try:
            ...
            if _custom_exe:
                log.info(f"Using custom Chromium binary: {_custom_exe}")
                self._context = await self._playwright.chromium.launch_persistent_context(
                    executable_path=_custom_exe, **launch_kwargs
                )
            else:
